...
<div class="flex flex-col md:flex-row mx-10 pt-10 mb-20">


<div class="mr-10 w-1/3">
<img src="{{$actor['profile_path']}}" class="w-76"/>
...
<div class="w-2/3">
    <p class="text-3xl font-bold mb-4">{{$actor['name']}} </p>
    <p class="text-sm mb-4">{{$actor['birthday']}} ({{$actor['age']}} years old) in {{$actor['place_of_birth']}} </p>
    <p class="font-bold mb-2">Known For</p>

    <div class="overflow-x-hidden">
    <div class="flex overflow-x-auto w-full">
    @foreach($combined_credits as $credit)
    <div class="mr-4">
    <a href="{{$credit['linkToPage']}}" class="mr-4">
    <img src="{{$credit['poster_path'] }}" class="h-32 w-auto"/>
    <p>{{$credit['title']}} </p>
    </a>
    </div>
    @endforeach
...
<div class="credits border-b border-gray-800">
    <div class="container mx-auto px-4 py-16">
        ...
        <ul class="list-disc leading-loose pl-5 mt-8">
            @foreach ($knownFor as $famous)
                <li>
                    {{ $famous['year'] }} &middot;
                    <strong><a href="{{ $famous['linkToPage'] }}" class="hover:underline">{{ $famous['title'] }}</a></strong>
                     as {{ $famous['character'] }}
                 </li>
            @endforeach
